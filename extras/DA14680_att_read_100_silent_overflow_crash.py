...
while True:
	...
	if data:
		...
		if pkt and (BTLE_SCAN_RSP in pkt) and pkt.AdvA == advertiser_address.lower():
			...
			# Yes, we're sending raw link layer messages in Python. Don't tell Bluetooth SUG as this is forbidden!!!
			...
			att_read_by_group.len = 100 # here we add padding to the att read by group request
